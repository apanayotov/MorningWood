...
public class LoginActivity extends Activity implements QBCallback, View.OnClickListener {
    ...
    private QBUser user = new QBUser("garry", "garry2892pass");
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        QBAuth.createSession((QBCallback) user, new QBCallbackImpl() {
            @Override
            public void onComplete(Result result) {
                if (result.isSuccess()) {
                    QBChatService.getInstance().loginWithUser(user, (SessionListener) LoginActivity.this);
        ...
        if (result.isSuccess()) {
            ...
            QBChatService.getInstance().loginWithUser(user, new SessionListener() {
                ...
                public void onDisconnectOnError(Exception exc) {
                    Log.i(TAG, "disconnect error when login", exc);
                }
            });
        } else {
